...
def send_message(server, message_type, payload=''):
    try:
        message = f'{message_type}|http://app2:9022|{payload}'
...
def handle_message(message):
    ...
    if message_type == 'TOKEN':
        ...
        for server in servers:
            server['token_holder'] = (server['address'] == 'http://app2:9022')
...
def get_next_server():
    global servers
    current_index = next((index for index, d in enumerate(servers) if d["address"] == 'http://app2:9022'), -1)
...
def check_server_alive(server):
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(2)
            s.connect(server)
            s.sendall(b'HEARTBEAT|http://app2:9022|')
            return True
    except socket.error:
        return False
...
def register_with_existing_services():
    ...
    for server in servers:
        if server['address'] != 'http://app2:9022':
            try:
                response = requests.post(f'{server["address"]}/check_registration', json={'address': 'http://app2:9022'})
                if response.status_code == 200:
                    registered = response.json().get('registered', False)
                    if not registered:
                        requests.post(f'{server["address"]}/register', json={'address': 'http://app2:9022'})
                else:
                    requests.post(f'{server["address"]}/register', json={'address': 'http://app2:9022'})
